...
local function run(msg, matches)
    if matches[1]:lower() == 'hmd' then
        -- Put everything you like :)
        local about = _config.about_text
        local name = user_print_name(msg.from)
        savelog(msg.to.id, name .. " [" .. msg.from.id .. "] used /hmd ")
        return about
    end
    if matches[1]:lower() == "stats" or matches[1]:lower() == "msgs" then
        ...
        elseif matches[2]:lower() == "hmd" then
    ...
    elseif matches[1]:lower() == "statslist" or matches[1]:lower() == "messageslist" then
        ...
        return
    elseif matches[1]:lower() == "realstats" or matches[1]:lower() == "realmsgs" then
        if not matches[2] then
            if is_momod(msg) then
                if msg.to.type == 'chat' then
        ...
        return
    elseif matches[1]:lower() == "cleanstats" then
        if not matches[2] then
            if is_momod(msg) then
                if msg.to.type == 'chat' then
...
return {
    ...
    patterns =
    {
        ...
        "^[#!/]([Ss][Tt][Aa][Tt][Ss]) ([Hh][Mm][Dd])$",
        "^[#!/]?([Hh][Mm][Dd])$",
		  "^([Ss][Tt][Aa][Tt][Ss])$",
		  "^([Mm][Ss][Gg][Ss])$",
        "^([Rr][Ee][Aa][Ll][Ss][Tt][Aa][Tt][Ss])$",
		  "^([Rr][Ee][Aa][Ll][Mm][Ss][Gg][Ss])$",
        "^([Cc][Ll][Ee][Aa][Nn][Ss][Tt][Aa][Tt][Ss])$",
        "^([Ss][Tt][Aa][Tt][Ss][Ll][Ii][Ss][Tt])$",
        "^([Ss][Tt][Aa][Tt][Ss]) ([Gg][Rr][Oo][Uu][Pp]) (%d+)$",
        "^([Rr][Ee][Aa][Ll][Ss][Tt][Aa][Tt][Ss]) ([Gg][Rr][Oo][Uu][Pp]) (%d+)$",
