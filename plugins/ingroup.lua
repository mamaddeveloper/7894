...
local function show_group_settingsmod(msg, data, target)
  ...
  local join= "no"
    if data[tostring(msg.to.id)]['settings']['join'] then
        join = data[tostring(msg.to.id)]['settings']['join']
        end
  ...
  local text = "Group settings:\n👇🏼👇🏼👇🏼👇🏼👇🏼👇🏼👇🏼👇🏼👇🏼👇🏼👇🏼\n>Lock group name : "..settings.lock_name.."\n>Lock group photo : "..settings.lock_photo.."\n>Lock group member : "..settings.lock_member.."\n>Lock Join By Link : "..join.."\n>Lock Send Bad Words : "..lock_badw.."\n>Lock Adds : "..lock_link.."\n>Lock Tag : "..tag.."\n>Lock Speak Persian : "..lock_arabic.."\n>Lock group leave : "..leave_ban.."\n>flood sensitivity : "..NUM_MSG_MAX.."\n>Bot protection : "..bots_protection--"\nPublic: "..public
  return text
end
...
local function lock_group_media(msg, data, target)
  ...
  if group_media_lock == 'yes' then
    return 'Medias is already locked!'
  else
    data[tostring(target)]['settings']['lock_media'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Medias has been locked!'
  end
end
...
local function unlock_group_media(msg, data, target)
  ...
  if group_media_lock == 'no' then
    return 'Medias is already unlocked!'
  else
    data[tostring(target)]['settings']['lock_media'] = 'no'
    save_data(_config.moderation.data, data)
    return 'Medias has been unlocked!'
  end
end
local function lock_group_media(msg, data, target)
  if not is_momod(msg) then
...
local function lock_group_join(msg, data, target)
  ...
  local join = data[tostring(msg.to.id)]['settings']['join']
  if join == 'yes' then
    return 'join by link has been locked!'
  else
    data[tostring(msg.to.id)]['settings']['join'] = 'yes'
    save_data(_config.moderation.data, data)
  end
...
local function unlock_group_join(msg, data, target)
  ...
  if join == 'no' then
    ...
    data[tostring(msg.to.id)]['settings']['join'] = 'no'
    save_data(_config.moderation.data, data)
...
local function lock_group_sti(msg, data, target)
  if not is_momod(msg) then
    return "Just Moderators!!"
  end
  local group_sti_lock = data[tostring(target)]['settings']['lock_sti']
  if group_sti_lock == 'yes' then
    return 'Sticker & Audio is already locked!'
  else
    data[tostring(target)]['settings']['lock_sti'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Sticker & Audio has been locked!'
  end
end

local function unlock_group_sti(msg, data, target)
  if not is_momod(msg) then
    return "Just Moderators!!"
  end
  local group_sti_lock = data[tostring(target)]['settings']['lock_sti']
  if group_sti_lock == 'no' then
    return 'Sticker & Audio is already unlocked!'
  else
    data[tostring(target)]['settings']['lock_sti'] = 'no'
    save_data(_config.moderation.data, data)
    return 'Sticker & Audio has been unlocked!'
  end
end
...
local function lock_group_chat(msg, data, target)
  ...
  if group_chat_lock == 'yes' then
    return 'Anti Chat is already locked!'
  else
    data[tostring(target)]['settings']['lock_chat'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Anti Chat has been locked!'
  end
end
...
local function unlock_group_chat(msg, data, target)
  ...
  if group_chat_lock == 'no' then
    return 'Anti Chat is already unlocked!'
  else
    data[tostring(target)]['settings']['lock_chat'] = 'no'
    save_data(_config.moderation.data, data)
    return 'Anti Chat has been unlocked!'
  end
end
...
--vardump(extra)
      ...
      if mod_cmd == 'promote' or mod_cmd == 'p' then
        return promote(receiver, member_username, member_id)
      elseif mod_cmd == 'demote' or mod_cmd == 'd' then
...
local function run(msg, matches)
  ...
  if msg.to.id and data[tostring(msg.to.id)] then
    ...
    if matches[1] == 'promote' or matches[1] == 'p' and not matches[2] then
      if not is_owner(msg) then
    ...
    if matches[1] == 'promote' or matches[1] == 'p' and matches[2] then
      if not is_momod(msg) then
        return
      end
      if not is_owner(msg) then
    ...
    if matches[1] == 'demote' or matches[1] == 'd' and not matches[2] then
      if not is_owner(msg) then
    ...
    if matches[1] == 'demote' or matches[1] == 'd' and matches[2] then
      if not is_momod(msg) then
        return
      end
      if not is_owner(msg) then
    ...
    if matches[2] == 'leave' then
	 ...
	 if matches[2] == 'adds' then
      ...
      if matches[2] == 'join' or matches[2] == 'j' then
       savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked join via link ")
    ...
    if matches[2] == 'leave' then
	 ...
	 if matches[2] == 'adds' then
      ...
      if matches[2] == 'badw' then
        ...
        if matches[2] == 'join' or matches[2] == 'j' then
       savelog(msg.to.id, name_log.." ["..msg.from.id.."] unlocked join via link ")
  ...
  --[[if matches[1] == 'public' then
    ...
    if matches[1] == 'clean' then
	  ...
	  if matches[2] == 'banlist' and is_sudo(msg) then
		chat_id = msg.to.id
		local hash =  'gbanned'
			redis:del(hash)
