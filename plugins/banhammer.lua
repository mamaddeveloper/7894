...
--vardump(extra)
      ...
      local receiver = "chat#id"..chat_id
       if get_cmd == "sik" or get_cmd == "k" then
      ...
      elseif get_cmd == 'ban' or get_cmd == 'b' then
        if is_momod2(member_id, chat_id) and not is_admin2(sender) then
          return send_large_msg(receiver, "You can't ban mods/owner/admins")
        end
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] banned')
        return ban_user(member_id, chat_id)
      elseif get_cmd == 'unban' or get_cmd == 'unb' then
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] unbanned')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'User '..user_id..' unbanned'
      elseif get_cmd == 'banall' or get_cmd == 'gb' then
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] globally banned')
        return banall_user(member_id, chat_id)
      elseif get_cmd == 'unbanall' or get_cmd == 'ungb' then
...
local function run(msg, matches)
 if matches[1]:lower() == 'id' then
    ...
    elseif matches[1]:lower() == 'id' then
      local name = user_print_name(msg.from)
      savelog(msg.to.id, name.." ["..msg.from.id.."] used /id ")
      return "👥Group ID for " ..string.gsub(msg.to.print_name, "_", " ").. ": "..msg.to.id.."\n🆔Your id: "..msg.from.id 
    end
  end
  ...
  if matches[1]:lower() == 'ban' or matches[1]:lower() == 'b' then-- /ban 
  ...
  if matches[1]:lower() == 'unban' or matches[1]:lower() == 'unb' then -- /unban 
    if type(msg.reply_id)~="nil" and is_momod(msg) then
...
if matches[1]:lower() == 'sik' or matches[1]:lower() == 'k' then
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      if is_admin(msg) then
  ...
  if matches[1]:lower() == 'banall' or matches[1]:lower() == 'gb' then -- Global ban
  ...
  if matches[1]:lower() == 'unbanall' or matches[1]:lower() == 'ungb' then -- Global unban
    local user_id = matches[2]
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then 
